...
export function chooseGame(set){
  ...
  group('count','Number of cards',CARD_OPTIONS.map(value=>[value,value]));group('per','Items per card',ITEM_OPTIONS.map(value=>[value,value]));group('mode','Card content',MODE_OPTIONS);group('movement','Movement',MOVEMENT_OPTIONS);group('theme','Theme',THEME_OPTIONS);
